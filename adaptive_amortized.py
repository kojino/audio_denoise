    ...
    for i in range(r):
        ...
        while ((fST - fS) < (ep/20)*(OPT - fS)) and (len(union(S,T)) < k):
            ...
            while (vs < (1-ep)*(OPT - fST)/r) :
                ...
                if parallel:
                    marg_a = Parallel(n_jobs=-1, verbose=50)(delayed(estimateMarginal)(X, union(S,T), a, m) for a in X)
                else:
                    marg_a = [estimateMarginal(X, union(S,T), a, m) for a in X]

                # Filter!
                Xnew = [X[idx] for idx, el in enumerate(marg_a) if el >= (1+ep/2)*(1-ep)*(OPT - fST)/k]
                X = Xnew

                # estimate if filtered set is good enough
                vs = estimateSet(X, union(S,T), m)
                if debug:
                    print('Elements remaining: ' + str(len(X)))
                    print('Check')
                    print(vs < (1-ep)*(OPT - fST)/r)
...
print('Num Features: ' + str(len(all_predictors)))
